...
if(isset($_SESSION['login_status']) && $_SESSION['login_status']==1) {
	...
	if(mysql_num_rows($rIfP)<1) {
		$insq = "insert into trades (user_id,sessid) values('".$userid."','".$sess_id."')";
		if(mysql_query($insq)) {
			$msg = "Successfull";
		} else {
			$msg = mysql_error();
...
if(isset($_POST['pay_type']) && $_POST['pay_type']=="paypal") {
...
if(isset($_POST['pay_type']) && $_POST['pay_type']=="cheque") {
	$chkadr = $_POST['chkadr'];
	$chqk = "select trade_id from trades where user_id = ".$userid." and sessid='".$sess_id."'";
	$chqr = mysql_query($chqk);
	if(mysql_num_rows($chqr)<1) {
...
if(isset($_POST['pay_type']) && $_POST['pay_type']=="charity") {
