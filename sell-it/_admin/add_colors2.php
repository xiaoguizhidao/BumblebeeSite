...
if(!empty($_FILES['color_image']) && ($_FILES['color_image']['error'] == 0)) {
		...
		$ext = substr($filename, strrpos($filename, '.') + 1);
			...
			$rsLastProduct=mysql_query("select max(c_id) as max_pid from add_extraimg limit 1");
			$rowLastProduct=mysql_fetch_array($rsLastProduct);
			$max_pid=$rowLastProduct['max_pid']+1;
			$changedFilename = $max_pid.$filename;
			$uploadat="../colors2/".$changedFilename;
			$uploadatthumb="../colors2/thumbs/".$changedFilename;
			$uploadatthumb_s="../colors2/thumbs/thumbs/".$changedFilename;
	...
	// has completed.
			//******************************thumbnail***************************************
				// This is the temporary file created by PHP 
				$uploadedfilethumb = $_FILES['color_image']['tmp_name'];
				...
				$newwidththumb=261;
				$newheightthumb=($heightthumb/$widththumb)*261;
			...
			//******************************thumbnail***************************************
				// This is the temporary file created by PHP 
				$uploadedfilethumb_s = $_FILES['color_image']['tmp_name'];
				
				// Create an Image from it so we can do the resize
				$srcthumb_s = imagecreatefromjpeg($uploadedfilethumb_s);
				
				// Capture the original size of the uploaded image
				list($widththumb_s,$heightthumb_s)=getimagesize($uploadedfilethumb_s);
				
				$newwidththumb_s=25;
				$newheightthumb_s=($heightthumb_s/$widththumb_s)*65;
				$tmpthumb_s=imagecreatetruecolor($newwidththumb_s,$newheightthumb_s);
				
				// this line actually does the image resizing, copying from the original
				// image into the $tmp image
				imagecopyresampled($tmpthumb_s,$srcthumb_s,0,0,0,0,$newwidththumb_s,$newheightthumb_s,$widththumb_s,$heightthumb_s); 
				
				// now write the resized image to disk. I have assumed that you want the
				// resized, uploaded image file to reside in the ./images subdirectory.
				$filenamethumb_s = $uploadatthumb_s;
				//echo $filenamethumb_s;
				imagejpeg($tmpthumb_s,$filenamethumb_s,100);
				
				imagedestroy($srcthumb_s);
				imagedestroy($tmpthumb_s); // NOTE: PHP will clean up the temp file it created when the request
				// has completed.
			//******************************************************************************
	}
//---------------------------------------------------------------------------------------------
$insert_qry=mysql_query("insert into add_extraimg (color_image,p_id) values('$changedFilename','$p_id')")or die(mysql_error());
}
?>
...
if (r==true)
  {
 window.location="add_colors2.php?c_id=<?php echo($rsColors['c_id']."&action=delete_image"); ?>";
  }
  else
  return false;
}
...
<table width="90%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td width="100%" align="left" valign="top" bgcolor="#FFFFFF"><table width="100%"  height="100%"border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td align="left" valign="top" background="images/usr_bg2.jpg" style="background-repeat:repeat-x;"><form action="<?php echo $_SERVER['includes/PHP_SELF'];?>?p_id=<?php echo $p_id;?>" method="post" enctype="multipart/form-data">
 ...
 <?php 
                if ($_REQUEST['action']=="delete_image"){
				  $c_id=$_GET['c_id'];
				  $del="delete from add_extraimg where c_id='$c_id'";
				  $delete=mysql_query($del) or die(mysql_error());
				}
 $gllery_qry=mysql_query("select * from add_extraimg where p_id='$p_id'")or die(mysql_error());
...
// if($num>0){
 ...
 ?>
  <tr>
    ...
    <td><img src="../colors2/thumbs/thumbs/<?php echo $row['color_image'];?>" width="70" height="70" /> </td>
   <!-- <td><a href="edit_gallery.php?img_id=<?php echo $row['img_id'];?>&p_id=<?php echo $p_id;?>">Edit</a></td>-->
    <td><a onClick="return confirmation()" href="add_colors2.php?c_id=<?php echo ($row['c_id']."&action=delete_image&p_id=".$row['p_id']); ?>">Delete</a></td>
  </tr>
  <?php }
  //}
  ?>
