...
if(isset($_POST['cid'])) {
	...
	$currencySign = addslashes($_POST['currencySign']);
	$currencyRate = addslashes($_POST['currencyRate']);
	$qryupd = "update currency set currency = '".$currencyName."', cur_sign = '".$currencySign."', cur_rate = '".$currencyRate."' where cur_id = ".$curid;
	if(mysql_query($qryupd)) {
		$msg = "Currency " .$currencyName. " Updated";
	} else {
		$msg = "error Updating ".mysql_error();
	}
}
?><?php
if (!isset($sRetry))
    ...
    <td width="100%" align="left" valign="top" bgcolor="#FFFFFF"><table width="100%"  height="100%"border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td align="left" valign="top" style="padding-top:15px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="borderbtm">
            <tr><td><a href="Currency.php">Manage Currency</a></td>
            <tr><td><?php echo $msg; ?></td>
            </tr>
            
            <tr><td>
            <table width="760" align="center">
            <tr><td>Currency</td><td>Currency Sign</td><td>Currency Rate</td><td>Update</td></tr>
            <?php
			$query = mysql_query("select * from currency order by cur_id asc");
			while($cur = mysql_fetch_array($query)) {
			?>
            <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <tr><td><input type="text" name="currencyName" value="<?php echo $cur['currency']; ?>"></td><td><input type="text" name="currencySign" value="<?php echo $cur['cur_sign']; ?>"></td><td><input type="text" name="currencyRate" value="<?php echo $cur['cur_rate']; ?>"></td><td><input type="submit" value="Update"><input type="hidden" name="cid" value="<?php echo $cur['cur_id']; ?>"></td></tr>
            </form>
            <?php } ?>
            </table>
            </td>
