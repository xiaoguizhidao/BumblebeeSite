...
if(isset($_REQUEST['sportsid'])) {
	$sportname = $_REQUEST['sports_name'];
	$sportssort = $_REQUEST['sort'];
	$sportsid = $_REQUEST['sportsid'];
	$qr = "update sports set sports_name='$sportname' ,sort='$sportssort' where sports_id=$sportsid";
	if(mysql_query($qr)) {
		header("location: sports.php");
	} else {
		$msg = "Error saving sports ".mysql_error();
	}
}


if(isset($_REQUEST['id'])) {
	$mysportsid = $_REQUEST['id'];
	$data = mysql_query("select * from sports where sports_id = $mysportsid");
	if(mysql_num_rows($data)>0) {
	$sports=mysql_fetch_array($data);
	} else {
	header("location: sports.php");
	}
} else {
    ...
    <td width="100%" align="left" valign="top" bgcolor="#FFFFFF"><table width="100%"  height="100%"border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td align="left" valign="top" style="padding-top:15px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="borderbtm">
            ...
            <tr><td>Name</td><td><input type="text" name="sports_name" value="<?php echo $sports['sports_name'] ?>"></td></tr>
            <tr><td>Sort Order</td><td><input type="text" name="sort" value="<?php echo $sports['sort'] ?>"></td></tr>
            <tr><td></td><td><input type="hidden" name="sportsid" value="<?php echo $sports['sports_id'] ?>"><input type="submit" value="Save"><input type="reset" value="Reset"></td></tr>
            </table>
            </form>
