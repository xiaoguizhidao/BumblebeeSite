...
?>
<?php 
$cat_qry=mysql_query("select * from clint_cat")or die(mysql_error());

$hn_id=$_GET['client_id'];
$qry=mysql_query("select * from clients where client_id='$hn_id'")or die(mysql_error());
...
$client_id=$_GET['client_id'];
	...
	$hn_des=addslashes($_POST['hn_des']);
		$cat=$_POST['cat'];
	$update_qry="update clients set client_name='$hn_name',client_des='$hn_des',cat_id='$cat'";
if ($_FILES["honey"]["error"] > 0)
 ...
 move_uploaded_file($_FILES["honey"]["tmp_name"],
      "../clients/" . $_FILES["honey"]["name"]);
$filename = basename($_FILES['honey']['name']);
$update_qry.=" ,client_img='$filename'";

}
$update_qry.=" where client_id='$client_id'";
//echo $update_qry;
...
echo($_SERVER['PHP_SELF']);?>?client_id=<?php echo $hn_id;?>" 
...
value="<?php echo $rs["client_name"]?>" size="26"></td>
              </tr>
...
rows="10" class="passfieds"><?php echo $rs["client_des"]?></textarea></td>
              </tr>
...
name="honey" id="honey" class="passfieds">
                <?php 
if($rs["client_img"]!=""){?>
<img src="../clients/<?php echo($rs['client_img']);?>" width="50" height="50" />
<?php }
?>(101 * 62)</td>
              </tr>
              <tr>
                <td>Category</td>
                <td><select name="cat" class="passfieds">
                <option value="">Select</option>
				<?php 
				while($row_cat=mysql_fetch_array($cat_qry)){?>
				<option value="<?php echo $row_cat['cat_id'];?>" <?php if($row_cat['cat_id']==$rs['cat_id']){echo "selected";}?>><?php echo $row_cat['cat_name'];?></option>
				<?php }
				?>
                </select></td>
              </tr>
